# SPESIFIKASI TABEL RELASI — SISTEM SIPADUHOK

> **SUMBER DATA OTORITATIF.** Seluruh isi dokumen ini dibangkitkan langsung dari `information_schema` basis data produksi `db_sipaduhok` (MySQL 8) — nama tabel, nama kolom, tipe data, primary key, dan foreign key **sesuai kondisi database yang sebenarnya**, bukan asumsi. Total tabel domain yang disajikan: **58**.

**Catatan cakupan.** Spesifikasi ini memuat seluruh tabel yang merupakan **entitas bisnis** hasil perancangan sistem. Selain tabel-tabel tersebut, basis data juga memiliki beberapa tabel **bawaan framework Laravel** yang dipakai untuk keperluan teknis, yaitu: `sessions` (penyimpanan sesi login), `password_reset_tokens` (token fitur lupa kata sandi), `cache` & `cache_locks` (penyimpanan cache), `jobs`, `failed_jobs`, & `job_batches` (antrian/queue proses latar belakang), serta `migrations` (pelacakan versi skema). Tabel-tabel ini **memang berfungsi dan digunakan sistem**, namun **tidak disertakan** dalam spesifikasi karena bersifat generik/infrastruktur dan bukan bagian dari rancangan basis data domain. Tabel `kelas_duplicate_backup` (cadangan) dan `promotion_tables` (tabel sisa tanpa data & tanpa referensi kode) juga dikecualikan. Perlu ditegaskan: fitur **kenaikan kelas** tetap tercakup penuh melalui tabel `status_naik_kelas_siswa`, `promotion_schedules`, `pengaturan_naik_kelas`, dan `izin_naik_kelas_khusus` pada Subsistem C.

**Legenda:** kolom bertanda **(PK)** = *Primary Key*, **(FK)** = *Foreign Key*. Kolom "Links to PK" menunjukkan tabel & kolom tujuan yang dirujuk oleh foreign key tersebut. Kolom yang boleh kosong ditandai `nullable` pada kolom *Extra*.
...
**Penjelasan:** *(Peran Pengguna.)* Master peran pengguna (9 peran) dengan `level` hierarki (1=tertinggi). Menjadi acuan `users.role_id`.
...
**Penjelasan:** *(Pengguna (Akun).)* Tabel induk semua akun (semua peran). Menyimpan kredensial, keamanan (PIN/pertanyaan), status aktif. Dirujuk hampir semua tabel lain.
...
**Penjelasan:** *(Cabang.)* Data cabang/lokasi sekolah (multi-cabang). Menjadi scope untuk kelas, siswa, dan user.
...
**Penjelasan:** *(Tenaga Pendidik (Guru/Pegawai).)* Profil guru/pegawai (1:1 dengan users). Dipakai sebagai guru pengajar, wali kelas, dan pembuat konten LMS.
...
**Penjelasan:** *(Siswa.)* Profil siswa (1:1 dengan users) beserta flag validasi akses ujian & rapor (bendahara/wali/ketua).
...
**Penjelasan:** *(Relasi Wali–Siswa.)* Tabel penghubung M:N wali/orang tua dengan siswa. Menandai penanggung jawab utama, penanggung keuangan, dan hak akses akademik.
...
**Penjelasan:** *(Tahun Ajaran.)* Master tahun ajaran; hanya satu yang aktif. Menjadi scope mayoritas data akademik & keuangan.
...
**Penjelasan:** *(Mata Pelajaran.)* Master mata pelajaran per jenjang (KB–SMA) dan kelompok A/B.
...
**Penjelasan:** *(Kelas.)* Rombongan belajar per tahun ajaran & cabang, memiliki wali kelas.
...
**Penjelasan:** *(Penugasan Guru Pengajar.)* Penugasan guru mengajar mapel tertentu di kelas tertentu (M:N guru–kelas–mapel).
...
**Penjelasan:** *(Penugasan Wali Kelas.)* Penugasan wali kelas (guru dengan kelas), unik per pasangan.
...
**Penjelasan:** *(Jadwal Pelajaran.)* Jadwal mengajar: mapel + guru + kelas pada hari & jam tertentu dalam satu tahun ajaran.
...
**Penjelasan:** *(Riwayat Perubahan Jadwal.)* Riwayat perubahan jadwal (audit trail: field, nilai lama/baru, alasan, oleh siapa).
...
**Penjelasan:** *(Relasi Jadwal–Kelas.)* Tabel penghubung jadwal dengan kelas untuk jadwal multi-kelas/gabungan.
...
**Penjelasan:** *(Pengaturan Jam Istirahat.)* Konfigurasi jam istirahat yang disisipkan pada tampilan jadwal.
...
**Penjelasan:** *(Presensi (Kehadiran).)* Rekap kehadiran siswa per mapel/kelas per tanggal, diinput oleh guru.
...
**Penjelasan:** *(Nilai Akademik.)* Nilai siswa per mapel per tahun ajaran (input guru, dapat diedit wali kelas dengan jejak editor).
...
**Penjelasan:** *(Pengaturan KKM.)* KKM (Kriteria Ketuntasan Minimal) per mapel per tahun ajaran.
...
**Penjelasan:** *(Rapor.)* Dokumen rapor siswa per tahun ajaran & kelas beserta status penerbitannya.
...
**Penjelasan:** *(Nilai per Mapel pada Rapor.)* Baris nilai per mapel di dalam sebuah rapor (menautkan rapor ke nilai & mapel).
...
**Penjelasan:** *(Kegiatan Ekstrakurikuler Rapor.)* Catatan kegiatan ekstrakurikuler yang tercetak pada rapor.
...
**Penjelasan:** *(Template Capaian Kompetensi.)* Template kalimat capaian kompetensi per mapel untuk mengisi deskripsi rapor.
...
**Penjelasan:** *(Pengajuan Rapor ke Ketua.)* Pengajuan penerbitan rapor yang membutuhkan persetujuan Ketua PKBM.
...
**Penjelasan:** *(Permintaan Unduh Rapor.)* Permintaan unduh rapor oleh siswa/wali beserta keputusan penyetujuan.
...
**Penjelasan:** *(Status Kenaikan Kelas Siswa.)* Status kenaikan/tinggal kelas siswa per tahun ajaran, mendukung rollback.
...
**Penjelasan:** *(Jadwal Eksekusi Kenaikan Kelas.)* Jadwal/periode proses kenaikan kelas per tahun ajaran.
...
**Penjelasan:** *(Pengaturan Kenaikan Kelas.)* Parameter kriteria kenaikan kelas (mis. nilai/kehadiran) per tahun ajaran.
...
**Penjelasan:** *(Dispensasi Kenaikan Kelas.)* Dispensasi kenaikan kelas khusus untuk siswa tertentu beserta pengaju & penyetuju.
...
**Penjelasan:** *(Pertemuan Pembelajaran.)* Pertemuan/pekan pembelajaran LMS per kelas–mapel–guru; menjadi wadah materi, tugas, dan ujian.
...
**Penjelasan:** *(Materi Pembelajaran.)* Materi/bahan ajar LMS yang diunggah guru pada suatu pertemuan.
...
**Penjelasan:** *(Tugas.)* Penugasan (assignment) LMS yang dibuat guru.
...
**Penjelasan:** *(Pengumpulan Tugas Siswa.)* Pengumpulan tugas oleh siswa (M:N tugas dengan siswa) beserta nilai.
...
**Penjelasan:** *(Ujian.)* Bank ujian/kuis LMS per kelas–mapel.
...
**Penjelasan:** *(Soal Ujian.)* Butir soal milik sebuah ujian.
...
**Penjelasan:** *(Sesi Ujian Siswa.)* Sesi pengerjaan ujian oleh seorang siswa (status, skor, soal aktif).
...
**Penjelasan:** *(Jawaban Siswa.)* Jawaban siswa atas tiap soal dalam sesi ujian.
...
**Penjelasan:** *(Status Soal per Sesi Ujian.)* Status per soal dalam sesi ujian (mis. dikunjungi/dijawab/ragu-ragu) untuk navigasi.
...
**Penjelasan:** *(Log Pengawasan Ujian.)* Log pengawasan/anti-curang selama siswa mengerjakan ujian online.
...
**Penjelasan:** *(Forum Diskusi.)* Topik diskusi LMS per kelas–mapel (opsional terkait pertemuan/konten).
...
**Penjelasan:** *(Balasan Forum.)* Balasan pada forum diskusi, mendukung balasan bertingkat (self-reference).
...
**Penjelasan:** *(Kelas Virtual (Meeting).)* Jadwal kelas virtual (Zoom/Google Meet) per kelas–mapel oleh guru.
...
**Penjelasan:** *(Tagihan.)* Tagihan keuangan siswa (SPP, uang pangkal, dll). Mendukung carryover tunggakan antar tahun via `tagihan_asal_id`/`dialihkan_ke_id`.
...
**Penjelasan:** *(Pembayaran.)* Transaksi pembayaran atas tagihan (manual/Midtrans) beserta status validasi & validator.
...
**Penjelasan:** *(Informasi Pembayaran.)* Informasi rekening/metode pembayaran yang ditampilkan ke pengguna.
...
**Penjelasan:** *(Pengaturan Batas Pembayaran.)* Konfigurasi batas/tenggat pembayaran per tahun ajaran.
...
**Penjelasan:** *(Log Audit Keuangan.)* Jejak audit setiap aksi keuangan (create/update/verify) dengan nilai lama/baru & metadata.
...
**Penjelasan:** *(Berita.)* Berita/artikel untuk landing page publik.
...
**Penjelasan:** *(Flyer/Poster.)* Flyer/poster promosi yang ditampilkan di situs publik.
...
**Penjelasan:** *(Pengumuman.)* Pengumuman internal/publik, dapat terkait kalender akademik.
...
**Penjelasan:** *(Kalender Akademik.)* Agenda/kalender akademik per tahun ajaran.
...
**Penjelasan:** *(Halaman Landing (CMS).)* Halaman CMS situs publik.
...
**Penjelasan:** *(Section Halaman Landing.)* Blok/section penyusun sebuah landing page (konten dinamis).
...
**Penjelasan:** *(Notifikasi.)* Notifikasi in-app per pengguna.
...
**Penjelasan:** *(Catatan/Pesan Internal.)* Pesan/catatan antar pengguna (pengirim dengan penerima).
...
**Penjelasan:** *(Penanda Baca Catatan.)* Penanda baca catatan per pengguna (M:N catatan dengan user).
...
**Penjelasan:** *(Catatan Monitoring.)* Catatan monitoring pembelajaran dari pengawas/guru terhadap kelas–mapel.
...
**Penjelasan:** *(Tiket Pemulihan Akun.)* Tiket pemulihan akun (lupa email/akses) per pengguna.
...
**Penjelasan:** *(Setelan Aplikasi.)* Setelan global aplikasi berbasis key-value (mis. gate LMS on/off).
