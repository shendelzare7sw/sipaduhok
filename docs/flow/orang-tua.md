...
> Semua menu Orang Tua **scoped per anak** (parameter `{siswa}` di URL — biasanya `siswaId`). Convention: route name di-prefix `orang-tua.`; view path relatif terhadap `resources/views/`. Semua method ada di satu controller `OrangTua/OrangTuaController.php`. Tidak ada CRUD master data — orang tua **konsumen + pembayar + monitor**, bukan input data akademik.

### Dashboard

**Index view**: `orang-tua/dashboard.blade.php` · **Controller**: `OrangTuaController@dashboard`
...
|---|---|---|---|---|---|
| Tagihan (per anak) | `orang-tua.tagihan.anak` | GET | `@tagihanAnak` | `orang-tua/tagihan/index.blade.php` | Cross-link ke halaman tagihan anak terpilih. |
| Rapor (per anak) | `orang-tua.rapor.anak` | GET | `@raporAnak` | `orang-tua/rapor/index.blade.php` | Cross-link ke halaman rapor anak. |
| Ajukan Izin (per anak) | `orang-tua.presensi.ajukan-izin` | GET | `@ajukanIzin` | `orang-tua/presensi/ajukan-izin.blade.php` | Cross-link ke form pengajuan izin. |
| Riwayat Izin (per anak) | `orang-tua.presensi.riwayat-izin` | GET | `@riwayatIzin` | `orang-tua/presensi/riwayat-izin.blade.php` | Cross-link ke riwayat pengajuan izin. |
...
**Index view**: `orang-tua/tagihan/index.blade.php` · **Controller**: `OrangTuaController@tagihanAnak`
...
| Tombol/Aksi | Route name | HTTP | Controller@method | View | Logika ringkas |
|---|---|---|---|---|---|
| Bayar (single tagihan) | `orang-tua.tagihan.bayar` | POST | `@prosesBayar` | redirect | Proses pembayaran 1 tagihan. Jika metode upload bukti: simpan `Pembayaran` status `pending` (menunggu validasi Bendahara). Jika Midtrans: redirect ke `pembayaran.snap`. |
| Bayar Terpilih (bulk) | `orang-tua.tagihan.bulk-pay` | POST | `@processBulkPay` | redirect | Bulk pay beberapa tagihan sekaligus → gabungkan jadi 1 `Pembayaran` Midtrans (efisien). Redirect ke Snap dengan `pembayaranId` baru. |
| Cetak Invoice | `orang-tua.pembayaran.invoice` | GET | `@cetakInvoice` | `orang-tua/tagihan/invoice.blade.php` | Layout cetak invoice 1 pembayaran (PDF-friendly). |

**Catatan**: Tunggakan TA lama yang **belum dialihkan** muncul read-only — orang tua tidak bisa bayar langsung sampai Bendahara/Admin menjalankan `carryover.execute` (lihat [bendahara.md §Tarik Tunggakan](bendahara.md#tarik-tunggakan-carryover)). Pembayaran online → `Pembayaran.metode = midtrans` & status = `pending` sampai callback `snap.finish` set jadi `lunas`. Webhook `midtrans.notification` (terdaftar di route global di luar grup orang-tua, tanpa auth/CSRF) menerima notifikasi resmi dari Midtrans untuk update status.

---

### Pembayaran Snap (Midtrans)

**Index view**: `orang-tua/pembayaran/snap.blade.php` · **Controller**: `OrangTuaController@snapPayment`
...
| Tombol/Aksi | Route name | HTTP | Controller@method | View | Logika ringkas |
|---|---|---|---|---|---|
| Lanjutkan Pembayaran (JS Snap.js) | (callback) `orang-tua.pembayaran.snap.finish` | GET | `@snapFinish` | redirect | Setelah user selesai bayar di popup Midtrans → callback ini set `Pembayaran` status sesuai response. |
| Continue Pembayaran (resume Snap) | `orang-tua.pembayaran.continue` | POST | `@continuePayment` | redirect/JSON | Untuk pembayaran yang sudah pernah di-Snap tapi belum selesai — re-trigger Snap dengan `snap_token` lama (atau generate baru bila expired). |
...
**Index view**: `orang-tua/rapor/index.blade.php` · **Controller**: `OrangTuaController@raporAnak`
...
| Tombol/Aksi | Route name | HTTP | Controller@method | View | Logika ringkas |
|---|---|---|---|---|---|
| Lihat Detail Rapor | `orang-tua.rapor.detail` | GET | `@detailRapor` | `orang-tua/rapor/detail.blade.php` | Tampilan rapor lengkap (nilai per mapel + deskripsi capaian + kehadiran + catatan wali). Read-only. Tombol **Request Download** muncul di bawah. |
| Request Download Rapor | `orang-tua.rapor.request-download` | POST | `@requestDownloadRapor` | redirect | Buat `RequestDownloadRapor` status `menunggu` → muncul di antrian Wali Kelas (`wali.rapor.request-download.index`). Wali approve → `downloadRapor` aktif 24 jam via token. |
| Download Rapor (via token) | `orang-tua.rapor.download` | GET | `@downloadRapor` | file download | Setelah approve, link berbasis token aktif 24 jam. Validasi: token valid, belum expired, milik orang tua ini. Stream PDF rapor. |
...
**Index view**: `orang-tua/presensi/index.blade.php` · **Controller**: `OrangTuaController@presensiAnak`
...
| Tombol/Aksi | Route name | HTTP | Controller@method | View | Logika ringkas |
|---|---|---|---|---|---|
| Ajukan Izin/Sakit | `orang-tua.presensi.ajukan-izin` | GET | `@ajukanIzin` | `orang-tua/presensi/ajukan-izin.blade.php` | Form pengajuan izin baru (lihat sub-section di bawah). |
| Simpan Pengajuan Izin | `orang-tua.presensi.store-izin` | POST | `@storeIzin` | redirect | Buat record `Presensi` status `pending` dengan upload bukti — dilempar ke Wali Kelas untuk divalidasi. |
| Riwayat Presensi (lengkap, lintas-bulan) | `orang-tua.presensi.riwayat-presensi` | GET | `@riwayatPresensi` | `orang-tua/presensi/riwayat-presensi.blade.php` | List semua presensi anak (filter tanggal). Read-only. |
| Riwayat Pengajuan Izin | `orang-tua.presensi.riwayat-izin` | GET | `@riwayatIzin` | `orang-tua/presensi/riwayat-izin.blade.php` | List semua pengajuan izin (Pending/Disetujui/Ditolak + alasan). |
| Edit Pengajuan (form) | `orang-tua.presensi.edit-izin` | GET | `@editIzin` | `orang-tua/presensi/edit-izin.blade.php` | Form edit pengajuan — **hanya bila masih `pending`** (belum divalidasi wali). |
| Update Pengajuan | `orang-tua.presensi.update-izin` | PUT | `@updateIzin` | redirect | Update keterangan/tanggal/bukti. Bila wali sudah validasi, gagal (validation). |
...
**View**: `orang-tua/presensi/ajukan-izin.blade.php`
...
**View**: `orang-tua/presensi/edit-izin.blade.php`
...
**View**: `orang-tua/tagihan/invoice.blade.php` · **Route**: `orang-tua.pembayaran.invoice` (dari `cetakInvoice`)
