        ...
        <ul class="space-y-1">
            @foreach($section['items'] as $item)
                @php
                    ...
                    $base = 'group flex min-h-11 w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left text-sm font-semibold transition';
                    $state = $isActive ? 'border-white/30 bg-white/20 text-white shadow-lg shadow-slate-950/15' : 'border-white/10 bg-white/[0.03] text-blue-50/90 hover:border-white/20 hover:bg-white/10 hover:text-white';
                @endphp
                <li class="menu-item {{ $isActive ? 'active open' : '' }}">
                    @if($hasChildren)
                        <button type="button" class="menu-link menu-toggle border-0 bg-transparent {{ $base }} {{ $state }}" data-menu-toggle aria-controls="{{ $targetId }}" aria-expanded="{{ $isActive ? 'true' : 'false' }}"><i class="fas {{ $item['icon'] }} w-5 text-center text-sm" aria-hidden="true"></i><span class="min-w-0 flex-1"><span class="block truncate">{{ $item['label'] }}</span><span class="mt-0.5 block truncate text-[10px] font-medium {{ $isActive ? 'text-sky-50' : 'text-sky-100/80 group-hover:text-white' }}">{{ $item['description'] }}</span></span><i class="fas fa-chevron-down text-[10px] transition-transform {{ $isActive ? 'rotate-180' : '' }}" data-menu-chevron aria-hidden="true"></i></button>
