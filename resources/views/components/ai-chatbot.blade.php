...
// ╔══════════════════════════════════════════════════════════════════╗
// ║  SIPADUHOK SYSTEM CHATBOT — NLP Engine (Server-Side)           ║
// ║  Default mode: NLP API dulu → jika out_of_scope → fallback LLM ║
// ╚══════════════════════════════════════════════════════════════════╝
...
function initChatbot() {
    ...
    setupEventListeners();
    initDraggableFab();
    console.log('SIPADUHOK Chatbot initialized (NLP System + LLM fallback)');
}
...
async function sendMessage(messageText = null) {
    ...
    // ═══ SYSTEM CHATBOT: send to NLP API FIRST (if no file attachments) ═══
    if (currentFiles.length === 0 && message) {
        ...
        try {
            ...
            const nlpResponse = await fetch('/ai-chatbot/nlp-message', {
                method: 'POST',
                headers: {
            ...
            // NLP API answered successfully and it's NOT out_of_scope
            if (nlpData.success && nlpData.tag !== 'out_of_scope') {
                chatbotState.isWaitingResponse = false;
                hideTypingIndicator();
                
                // Cek apakah ada UI Data terstruktur dari NLP API
                let finalHtml = nlpData.response;
                let isHtml = false;
                
                if (nlpData.ui_data) {
                    finalHtml = buildRichUiHtml(nlpData.response, nlpData.ui_data);
                    isHtml = true;
                } else if (nlpData.role_mismatch) {
                    // Tampilkan blok peringatan larangan akses lintas role
                    finalHtml = `<div style="font-size:13.5px;line-height:1.6;color:#475569;font-family:inherit;">
                        <div style="font-weight:700;color:#ef4444;margin-bottom:8px;"><i class="fas fa-lock" style="margin-right:6px"></i> Otorisasi Terbatas</div>
                        ${nlpData.response.replace(/\n\n/g, '<br><br>')}
                    </div>`;
                    isHtml = true;
                }
                
                addMessage('assistant', finalHtml, null, true, isHtml);
                saveCurrentConversation();
                scrollToBottom();
                return; // answered by NLP system — no LLM fallback needed
            }

            // NLP said out_of_scope → fall through to LLM below
        } catch (nlpError) {
            console.warn('[Chatbot] NLP API unavailable, falling through to LLM:', nlpError.message);
            // NLP API is down → fall through to LLM below
        }
    ...
    // ═══ NLP out_of_scope or unavailable → fallback to LLM API ═══
...
// ==================== Render Rich UI (NLP Data) ====================
function buildRichUiHtml(textResponse, ui) {
    let html = '<div style="font-size:13.5px;line-height:1.6;font-family:inherit;">';
    
    // Header
    if (ui.title) {
        html += `<div style="font-weight:700;color:#3b82f6;margin-bottom:8px;font-size:14.5px;display:flex;align-items:center;">
            ${ui.icon ? `<i class="${ui.icon}" style="margin-right:8px;font-size:15px;"></i>` : ''}${ui.title}
        </div>`;
    }
    
    // Deskripsi natural
    if (textResponse) {
        html += `<div style="color:#475569;margin-bottom:12px;">${textResponse}</div>`;
    }
    
    // Steps (No gray box, just plain text)
    if (ui.steps && ui.steps.length > 0) {
        html += `<div style="margin-bottom:14px;">`;
        ui.steps.forEach((step, index) => {
            html += `<div style="margin-bottom:4px;display:flex;">
                <span style="color:#475569;margin-right:6px;">${index + 1}.</span>
    ...
    // Note/Warning
    if (ui.note) {
        html += `<div style="padding:10px 14px;background:#fef3c7;border-left:4px solid #f59e0b;border-radius:0 6px 6px 0;font-size:12.5px;color:#92400e;margin-bottom:16px;">
            ${ui.note}
        </div>`;
    }
    
    // URL Button (Pill shape, icon on left, vibrant blue/indigo)
    if (ui.url) {
        html += `<div style="margin-bottom:16px;">
            <a href="${ui.url}" style="display:inline-flex;align-items:center;background:#4f46e5;color:white;padding:7px 18px;border-radius:100px;text-decoration:none;font-weight:600;font-size:12.5px;transition:all 0.2s;box-shadow:0 4px 6px -1px rgba(79,70,229,0.1);line-height:1;">
                <i class="fas fa-arrow-right" style="margin-right:8px;font-size:11px;vertical-align: middle;"></i> 
                <span style="vertical-align: middle;">${ui.urlText || 'Buka Menu'}</span>
            </a>
        </div>`;
    }
    
    // Related Tags (Outline style)
    if (ui.related && ui.related.length > 0) {
        html += `<div style="border-top:1px dashed #e2e8f0;padding-top:12px;display:flex;align-items:center;flex-wrap:wrap;gap:8px;">
            <div style="font-size:11.5px;color:#64748b;"><i class="fas fa-link" style="margin-right:4px;"></i> Terkait:</div>
            <div style="display:flex;flex-wrap:wrap;gap:6px;">`;
        ui.related.forEach(tag => {
            html += `<span style="background:#eff6ff;color:#3b82f6;padding:3px 12px;border-radius:100px;font-size:11.5px;border:1px solid #bfdbfe;cursor:pointer;transition:all 0.2s;" onclick="sendQuickAction('${tag}')" onmouseover="this.style.background='#dbeafe'" onmouseout="this.style.background='#eff6ff'">${tag}</span>`;
        });
        html += `</div></div>`;
    }
