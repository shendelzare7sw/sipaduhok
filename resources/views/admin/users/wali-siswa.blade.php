...
@section('page-title', 'Data Wali Siswa')
@section('page-subtitle', 'Kelola akun login wali siswa siswa')
...
@section('styles')
    @vite(['resources/css/admin/users/wali-siswa.css'])
@endsection
        ...
        <div class="card">
            <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-3">
                ...
                <div class="d-flex flex-wrap align-items-center gap-3 w-100-mobile">
                    ...
                    <div class="d-flex gap-2 align-items-center justify-content-between w-100-mobile">
                        <div class="d-flex gap-2 align-items-center">
                            ...
                            <div>
                                <h5 class="mb-0 fw-bold text-dark">Daftar Wali Siswa</h5>
                                <small class="text-muted">Total: {{ $orangTua->total() }} akun wali siswa</small>
                            </div>
                        </div>
                    </div>

                    {{-- Filter Form --}}
                    <form action="{{ route('admin.users.wali-siswa') }}" method="GET" id="filterForm" class="search-form d-flex gap-2 align-items-center w-100-mobile">
                        {{-- Filter Dropdown --}}
                ...
                <div class="d-flex gap-2 action-group-mobile">
                    <form action="{{ route('admin.users.bulk-delete-wali-siswa') }}" method="POST" id="bulkDeleteForm" class="bulk-delete-form">
                        @csrf
                    ...
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                                <a href="{{ route('admin.users.wali-siswa.print') }}?{{ http_build_query(request()->all()) }}" class="dropdown-item" target="_blank">
                                    <i class="fas fa-print me-2"></i> Cetak Data (PDF)
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('admin.users.import-wali-siswa') }}" class="dropdown-item">
                                    <i class="fas fa-file-import me-2"></i> Import Excel
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('admin.users.wali-siswa-template') }}" class="dropdown-item">
                                    <i class="fas fa-download me-2"></i> Download Template
                                </a>
                            </li>
                        </ul>
                    </div>
                    <a href="{{ route('admin.users.wali-siswa.create') }}" class="btn-primary btn-nowrap">
                        <i class="fas fa-plus"></i>
                        <span class="d-none d-md-inline">Tambah Wali Siswa</span>
                                ...
                                <td class="mobile-hide row-number">
                                    ...
                                    <td class="mobile-card-actions">
                                        <div class="action-buttons">
                                            {{-- Detail Button --}}
                                            <a href="{{ route('admin.users.show-wali-siswa', $ortu->id) }}" class="action-btn"
                                                title="Lihat Detail">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            {{-- Edit Button --}}
                                            <a href="{{ route('admin.users.edit-wali-siswa', $ortu->id) }}" class="action-btn"
                                                title="Edit Data">
                                    ...
                                    <td colspan="6" class="empty-table-cell">
                                        ...
                                        <div class="empty-table-title">
                                            @if(request('search') || request('jenjang') || request('cabang_id') || request('status'))
                                                Tidak ada data wali siswa yang sesuai dengan filter yang dipilih
                                            @else
                                        ...
                                        <small>
                                            @if(request('search') || request('jenjang') || request('cabang_id') || request('status'))
                                                Coba filter lain atau <a href="{{ route('admin.users.wali-siswa') }}"
                                                    class="reset-filter-link">hapus semua filter</a>
                                            @else
                                                Wali siswa akan terdaftar otomatis saat menambahkan siswa baru
                                            @endif
                                        </small>
                                    </td>
                                </tr>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <p>Apakah Anda yakin ingin {{ $ortu->is_active ? 'menonaktifkan' : 'mengaktifkan' }} akun wali siswa:
                            </p>
                            <div class="info-box">
                                <div class="info-name">
                            ...
                            @if($ortu->is_active)
                                <p class="modal-note">
                                    ...
                                    <small class="text-muted">Wali siswa yang dinonaktifkan tidak dapat login ke sistem.</small>
                                </p>
                            @else
                                <p class="modal-note">
                                    <i class="fas fa-info-circle text-success"></i>
                                    <small class="text-muted">Wali siswa yang diaktifkan dapat login dan mengakses sistem
                                        kembali.</small>
                                </p>
                            @endif
                        </div>
                        <div class="modal-footer bg-light">
                            ...
                            <form action="{{ route('admin.users.toggle-wali-siswa-status', $ortu->id) }}" method="POST"
                                class="d-inline">
                                @csrf
            ...
            <div class="modal fade" id="deleteModal{{ $ortu->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <p>Apakah Anda yakin ingin menghapus akun wali siswa:</p>
                            <div class="info-box">
                                <div class="info-name">
                                    <i class="fas fa-user text-primary"></i>
                                    {{ $ortu->name }}
                        ...
                        <div class="modal-footer bg-light">
                            ...
                            <form action="{{ route('admin.users.delete-wali-siswa', $ortu->id) }}" method="POST"
                                class="d-inline">
                                @csrf
                                @method('DELETE')
