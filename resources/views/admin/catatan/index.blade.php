...
.stat-card {
    ...
    height: 100%;
    color: white !important; /* Paksa teks putih agar kontras */
    border: none;
}
.stat-card:hover { transform: translateY(-5px); }
.stat-content { position: relative; z-index: 2; }
.stat-title { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; opacity: 0.9; margin-bottom: 8px; color: white !important; }
.stat-number { font-size: 28px; font-weight: 800; margin-bottom: 4px; line-height: 1; color: white !important; }
.stat-desc { font-size: 13px; opacity: 0.8; color: white !important; }
.stat-icon-bg { position: absolute; right: 15px; top: 50%; transform: translateY(-50%); font-size: 60px; opacity: 0.15; z-index: 1; color: white !important; }
...
.border-biasa   { border-left-color: #4e73df; }
.border-penting { border-left-color: #f6c23e; }
.border-mendesak{ border-left-color: #e74a3b; }

.catatan-title { font-size: 18px; font-weight: 700; color: #4e73df; }
.catatan-meta { font-size: 12px; color: #858796; display: flex; gap: 15px; align-items: center; }
...
.badge-semua { background: #eef2ff; color: #4338ca; border: 1px solid #c7d2fe; }
.badge-role { background: #ecfdf5; color: #065f46; border: 1px solid #a7f3d0; }
.badge-individu { background: #fff1f2; color: #be123c; border: 1px solid #fecdd3; }
...
<div class="container-fluid px-0" style="max-width: 1400px; margin: 0 auto; padding: 0 1rem;">
    ...
    {{-- Alert Success --}}
    @if(session('success'))
        <div class="alert alert-success border-left-success shadow show mb-4 animate__animated animate__fadeIn">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-white">
            <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-history mr-2"></i>Riwayat Catatan Instruksi</h6>
            <a href="{{ route('admin.catatan.create') }}" class="btn btn-primary btn-sm shadow-sm font-weight-bold">
        ...
        <div class="card-body">
                        ...
                        <div class="catatan-item border-{{ $cat->prioritas }}">
                            <div class="d-flex justify-content-between align-items-start mb-3">
                                <div>
                                    ...
                                    <div class="catatan-meta">
                                        <span><i class="far fa-clock mr-1 text-info"></i> {{ $cat->tanggal_kirim->format('d M Y, H:i') }}</span>
                                        <span>
                                            <i class="fas fa-layer-group mr-1 text-warning"></i>
                                            <span class="text-uppercase font-weight-bold" style="letter-spacing: 0.5px;">{{ $cat->prioritas }}</span>
                                        </span>
                                    </div>
                                </div>
                                <div>
                                    @if($cat->tipe_penerima === 'semua')
                                        <span class="badge-pill-custom badge-semua"><i class="fas fa-broadcast-tower mr-1"></i> SEMUA PENGGUNA</span>
                                    @elseif($cat->tipe_penerima === 'role')
                                        <span class="badge-pill-custom badge-role"><i class="fas fa-users mr-1"></i> ROLE: {{ strtoupper(str_replace('_', ' ', $cat->role_penerima)) }}</span>
                                    @else
                                        <span class="badge-pill-custom badge-individu"><i class="fas fa-user mr-1"></i> INDIVIDU: {{ $cat->penerima->name ?? 'User' }}</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="read-box">
                                    ...
                                    <span>Telah dibaca oleh <strong class="text-success">{{ $cat->totalPembaca() }}</strong> pengguna</span>
                                </div>
                                <a href="{{ route('admin.catatan.show', $cat->id) }}" class="btn btn-light border btn-sm font-weight-bold text-primary shadow-sm">
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <a href="{{ route('admin.catatan.create') }}" class="btn btn-primary mt-3 px-4">
