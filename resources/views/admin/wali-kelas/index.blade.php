...
@section('styles')
    @vite(['resources/css/admin/wali-kelas/index.css'])
@endsection
    ...
    <div class="wk-card">
        <div class="wk-card-header">
            ...
            <div class="header-actions d-flex gap-2">
                <a href="{{ route('admin.wali-kelas.print', request()->query()) }}" class="btn btn-secondary text-white d-flex align-items-center gap-1" target="_blank">
        ...
        <!-- Filters Form -->
        <form action="{{ route('admin.wali-kelas.index') }}" method="GET" class="mb-0">
            <div class="filter-wrapper">
                ...
                @if(request()->hasAny(['search', 'jenjang', 'cabang_id', 'status']) || (request('tahun_ajaran_id') && request('tahun_ajaran_id') != $currentTahunAjaran?->id))
                    <a href="{{ route('admin.wali-kelas.index', ['tahun_ajaran_id' => request('tahun_ajaran_id')]) }}" class="btn btn-outline-danger btn-sm px-3 filter-action-btn">
                    ...
                    <tr>
                        ...
                        <td class="td-actions text-end" data-label="Aksi">
                            <div class="d-flex justify-content-end gap-1 action-btns">
                                ...
                                <a href="{{ route('admin.wali-kelas.show', $kelas) }}" class="btn btn-sm btn-info text-white px-2 py-1 rounded" title="Lihat Detail">
                ...
                <div class="modal-body px-4 py-4">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex flex-column flex-sm-row gap-2">
                            ...
                            <select id="filterCabang" class="form-select modal-filter-select">
                                <option value="">Semua Cabang</option>
                                            ...
                                            @if($hasAssignments)
                                                <div class="d-flex flex-wrap gap-1 mt-1">
                                                    @foreach($assignedKelasList as $assignment)
                                                        {{-- Jaring pengaman: ->kelas bisa null kalau kelasnya sudah
                                                             dihapus. Halaman jangan sampai pecah cuma karena satu
                                                             data tidak lengkap. --}}
                                                        @if($assignment->kelas)
...
@section('scripts')
    @vite(['resources/js/admin/wali-kelas/index.js'])
@endsection
