...
@section('content')
@php
    $pembayaranRoute = request()->routeIs('bendahara.*') ? 'bendahara.pembayaran' : 'admin.keuangan.pembayaran';
...
<div class="min-w-0 w-full space-y-5" data-payment-index x-data="{ query: @js($filters['search'] ?? '') }">
    ...
    <section class="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <form action="{{ route($pembayaranRoute.'.index') }}" method="GET" class="grid gap-2 border-b border-slate-200 p-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-[minmax(220px,1.2fr)_repeat(3,minmax(150px,0.55fr))_repeat(2,minmax(145px,0.5fr))_auto] sm:p-5">
        ...
        @else
            ...
            <div class="hidden overflow-x-auto lg:block"><table class="w-full min-w-[1080px] table-fixed text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-100">@foreach($pembayaranList as $index => $pembayaran)<tr class="hover:bg-slate-50/70">
                    ...
                    <td class="px-4 py-4"><div class="flex justify-end"><x-cleanflow.table-action href="{{ route($pembayaranRoute.'.show', $pembayaran->id) }}" tone="view" icon="fas fa-eye" label="Detail pembayaran" /></div></td>
                </tr>@endforeach</tbody>
            </table></div>
