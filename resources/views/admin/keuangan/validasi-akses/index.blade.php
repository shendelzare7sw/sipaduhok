...
@section('content')
    <div style="max-width: 1400px; margin: 0 auto; padding: 0 1rem;">
        <div class="container-fluid px-0">
            ...
            <div class="card shadow mb-4" style="position: relative; z-index: 99;">
                <div class="card-body">
                    <div class="row align-items-center">
                        <div class="col-lg-8">
                            <form action="{{ route('admin.keuangan.validasi-akses.index') }}" method="GET" class="d-flex gap-2">
                                {{-- Filter Dropdown --}}
                                <div class="dropdown">
                        ...
                        <div class="col-lg-4 text-end">
                            <button type="button" class="btn btn-success btn-sm shadow-sm fw-bold"
                                onclick="bulkValidasiUjian()">
                                <i class="fas fa-check-double me-1"></i> Validasi Ujian
                            </button>
                            <button type="button" class="btn btn-info btn-sm shadow-sm fw-bold ms-1"
                                onclick="bulkValidasiRapor()">
                                    ...
                                    @forelse($siswa as $index => $s)
                                        <tr>
                                            <td class="text-center align-middle"><input type="checkbox" name="siswa_ids[]"
                                                    value="{{ $s->id }}" class="siswa-checkbox form-check-input"></td>
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3 bg-white">
                    <h6 class="m-0 fw-bold text-primary"><i class="fas fa-bolt me-2 text-warning"></i>Validasi Kilat Per
                        Kelas</h6>
                </div>
                <div class="card-body">
                    <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                <div class="card border-start border-primary border-4 shadow-sm h-100">
                                    <div class="card-body py-3">
                                        ...
                                        <div class="text-xs text-muted mb-3">{{ $kelas->jenjang }} •
                                            {{ $kelas->siswa->count() }} Siswa</div>
                                        <div class="row g-2">
                                            <div class="col">
                                                <form id="form-ujian-{{ $kelas->id }}"
                                                    action="{{ route('admin.keuangan.validasi-akses.bulk-validasi-ujian', $kelas->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="button" class="btn btn-success btn-sm w-100 fw-bold"
                                                        onclick="confirmClassAction('form-ujian-{{ $kelas->id }}', 'Validasi Ujian Se-Kelas', 'Validasi ujian untuk seluruh siswa di kelas {{ $kelas->nama_kelas }}?')">UJIAN</button>
                                                </form>
                                            </div>
                                            <div class="col">
                                                <form id="form-rapor-{{ $kelas->id }}"
                                                    action="{{ route('admin.keuangan.validasi-akses.bulk-validasi-rapor', $kelas->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="button" class="btn btn-info btn-sm w-100 fw-bold"
                                                        onclick="confirmClassAction('form-rapor-{{ $kelas->id }}', 'Validasi Rapor Se-Kelas', 'Validasi rapor untuk seluruh siswa di kelas {{ $kelas->nama_kelas }}?')">RAPOR</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
