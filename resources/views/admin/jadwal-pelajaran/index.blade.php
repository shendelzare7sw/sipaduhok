...
.hari-senin { background: #dbeafe; color: #1e40af; }
.hari-selasa { background: #dcfce7; color: #166534; }
.hari-rabu { background: #fef3c7; color: #92400e; }
.hari-kamis { background: #fce7f3; color: #9f1239; }
.hari-jumat { background: #e0e7ff; color: #3730a3; }
.hari-sabtu { background: #f3e8ff; color: #6b21a8; }
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-3">
        ...
        <div class="d-flex gap-2 flex-wrap">
            ...
            <a href="{{ route('admin.jadwal-pelajaran.export-pdf', request()->query()) }}" class="btn btn-danger btn-sm" target="_blank">
                <i class="fas fa-file-pdf me-1"></i> Export PDF
            </a>
            <a href="{{ route('admin.jadwal-pelajaran.export-excel', request()->query()) }}" class="btn btn-success btn-sm">
                <i class="fas fa-file-excel me-1"></i> Export Excel
            </a>
            <button type="button" class="btn btn-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#bulkReplaceModal">
        ...
        @if($jadwalList->count() > 0)
            <form id="bulk-form-jadwal">
                ...
                <div class="mb-3 d-flex gap-2 align-items-center">
                    ...
                    <button type="button" class="btn btn-warning btn-sm" onclick="bulkUpdateStatus()" id="bulkStatusBtn" style="display: none;">
            ...
            <div class="table-responsive">
                <table class="table table-hover jadwal-table">
                    <thead>
                        <tr>
                            <th style="width: 40px;">
                                <input type="checkbox" id="select-all-jadwal" class="form-check-input" onclick="toggleSelectAllJadwal()">
                            </th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-center align-middle">
                                <input type="checkbox" name="jadwal_ids[]" value="{{ $jadwal->id }}" class="form-check-input jadwal-checkbox" onchange="updateBulkButtons()">
                            </td>
                            <td>
                            ...
                            <td>
                                @if($jadwal->guru)
                                    <div class="guru-info">
                                        <div class="guru-avatar-sm">{{ strtoupper(substr($jadwal->guru->nama_lengkap, 0, 1)) }}</div>
                            ...
                            <td style="text-align: center;">
                                <div class="btn-group" role="group">
                                    <button type="button" class="btn btn-sm btn-info"
                                            ...
                                            data-bs-target="#gantiGuruModal{{ $jadwal->id }}"
                                            title="Ganti Guru">
...
@foreach($jadwalList as $jadwal)
                ...
                <div class="modal-body">
                    ...
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545;">
                        ...
                        <div style="font-size: 13px; color: #6c757d; display: flex; flex-direction: column; gap: 4px;">
                            ...
                            <div>
                                <i class="fas fa-clock me-1"></i> Jam: <strong>{{ \Carbon\Carbon::parse($jadwal->jam_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($jadwal->jam_selesai)->format('H:i') }}</strong>
                            </div>
                            @if($jadwal->guru)
                                <div>
                                    <i class="fas fa-user-tie me-1"></i> Guru: <strong>{{ $jadwal->guru->nama_lengkap }}</strong>
                                </div>
                            @endif
                        </div>
                    </div>
    ...
    {{-- Modal Ganti Guru --}}
    <div class="modal fade" id="gantiGuruModal{{ $jadwal->id }}" tabindex="-1" aria-labelledby="gantiGuruModalLabel{{ $jadwal->id }}" aria-hidden="true">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <div style="background: #f8f9fa; padding: 12px; border-radius: 6px; font-size: 13px;">
                                <div><i class="fas fa-calendar-day me-2 text-primary"></i><strong>{{ $jadwal->hari }}</strong>, {{ \Carbon\Carbon::parse($jadwal->jam_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($jadwal->jam_selesai)->format('H:i') }}</div>
                                <div class="mt-1"><i class="fas fa-school me-2 text-success"></i>{{ $jadwal->kelas->nama_kelas }}</div>
                            </div>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Guru Baru <small class="text-muted fw-normal">(Kosongkan untuk set jadwal kosong)</small></label>
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Alasan Penggantian <small class="text-muted fw-normal">(Opsional)</small></label>
...
<div class="modal fade" id="bulkReplaceModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('admin.jadwal-pelajaran.bulk-replace-guru') }}" method="POST">
            @csrf
            <input type="hidden" name="tahun_ajaran_id" value="{{ request('tahun_ajaran_id', $currentTahunAjaran?->id) }}">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Guru Baru <small class="text-muted">(Kosongkan untuk set semua jadwal kosong)</small></label>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning">
                        <i class="fas fa-exclamation-triangle me-2"></i>
                        <strong>Perhatian:</strong> Jadwal hanya akan di-copy untuk kelas yang memiliki nama dan jenjang yang sama.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
...
<div class="modal fade" id="bulkDeleteModal" tabindex="-1" aria-labelledby="bulkDeleteModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="mb-3">Apakah Anda yakin ingin menghapus <strong><span id="bulkDeleteCount">0</span> jadwal</strong> yang dipilih?</p>
...
<div class="modal fade" id="bulkStatusModal" tabindex="-1" aria-labelledby="bulkStatusModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="mb-3">Ubah status untuk <strong><span id="bulkStatusCount">0</span> jadwal</strong> yang dipilih:</p>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Pilih Status Baru:</label>
                    <div class="d-flex gap-3">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status_choice" id="statusAktif" value="aktif" checked>
                            <label class="form-check-label" for="statusAktif">
                                <span class="badge bg-success">AKTIF</span>
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status_choice" id="statusKosong" value="kosong">
