...
<div class="post {{ $level > 0 ? 'reply-level-' . min($level, 5) : '' }}"
            ...
            <div class="media-attachment mt-2">
                @if(Str::startsWith($reply->attachment_type, 'image/'))
                    <a href="{{ asset('storage/' . $reply->attachment) }}" target="_blank">
                        <img src="{{ asset('storage/' . $reply->attachment) }}" alt="Attachment" class="img-fluid rounded forum-media-preview">
                    </a>
                @elseif(Str::startsWith($reply->attachment_type, 'video/'))
                    <video controls class="w-100 rounded forum-media-preview">
                ...
                @elseif(Str::endsWith(strtolower($reply->attachment), '.pdf'))
                    <iframe src="{{ asset('storage/' . $reply->attachment) }}" class="forum-pdf-frame"></iframe>
                    <div class="mt-1">
                        <a href="{{ asset('storage/' . $reply->attachment) }}" target="_blank"
    ...
    @if(!$forum->is_closed)
        <button type="button" class="reply-btn" data-toggle-reply="reply-{{ $reply->id }}">REPLY</button>
    @endif
    ...
    <div id="reply-{{ $reply->id }}" class="reply-form">
        <form action="{{ route('guru.lms.forum.reply', [$kelas->id, $mapel->id, $forum->id]) }}" method="POST"
            ...
            {{-- Attachment Dropzone --}}
            <button type="button" class="attachment-toggle-btn" data-toggle-attachment-area="attachment-area-reply-{{ $reply->id }}">
            ...
            <div id="attachment-area-reply-{{ $reply->id }}" class="attachment-area">
                <div class="dropzone-wrapper">
                    ...
                    <input type="file" name="attachment" class="dropzone-file-input"
                        data-file-name-target="file-name-reply-{{ $reply->id }}">
                </div>
                <div id="file-name-reply-{{ $reply->id }}" class="file-preview-name"></div>
            </div>
            <div class="text-end">
                <button type="button" class="btn btn-secondary btn-sm me-2"
                    data-toggle-reply="reply-{{ $reply->id }}">Batal</button>
