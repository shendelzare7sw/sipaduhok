...
<div class="post {{ $level > 0 ? 'reply-level-' . min($level, 5) : '' }}" data-author="{{ $reply->user->name }}"
    ...
    <div class="post-header">
        ...
        <div class="post-info">
            <div>
                ...
                @if($isMe)
                    <span class="badge bg-secondary badge-me">Anda</span>
                @endif
            </div>
            <div class="post-date">
    ...
    <div class="post-content" id="content-{{ $uniqueId }}">
        ...
        @if($isMe)
            <div class="post-actions d-flex gap-2">
                <button type="button" class="action-icon text-primary" data-toggle-reply="edit-form-{{ $uniqueId }}" title="Edit">
                    <i class="fas fa-pencil-alt"></i>
                </button>
                <button type="button" class="action-icon text-danger" title="Hapus" 
                    data-delete-reply-url="{{ route('guru.lms.forum.reply.destroy', [$kelas->id, $mapel->id, $forum->id, $reply->id]) }}">
    ...
    @if(!$forum->is_closed)
        <button type="button" class="reply-btn" data-toggle-reply="form-{{ $uniqueId }}">
