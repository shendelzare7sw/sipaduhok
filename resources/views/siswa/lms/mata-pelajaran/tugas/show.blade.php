...
<div class="deadline-box {{ $isExpired ? 'expired' : ($hoursLeft < 24 ? '' : 'safe') }}">
    ...
    <div style="font-size: 16px; margin-top: 10px;">
        ...
        @elseif($hoursLeft < 24)
            <i class="fas fa-exclamation-triangle"></i> Segera dikumpulkan! ({{ $hoursLeft }} jam lagi)
        @else
            <i class="fas fa-check-circle"></i> Tersisa {{ $daysLeft }} hari lagi
        @endif
    </div>
</div>
...
<div class="tugas-card">
    ...
    <div style="border-bottom: 2px solid #e5e7eb; padding-bottom: 20px; margin-bottom: 25px;">
        ...
        <div style="display: flex; gap: 15px; flex-wrap: wrap; color: #666; font-size: 14px;">
            ...
            <div><i class="fas fa-calendar-plus"></i> <strong>Dibuka:</strong> {{ $tugas->tanggal_mulai->format('d M Y') }}</div>
            <div><i class="fas fa-calendar-times"></i> <strong>Ditutup:</strong> {{ $tugas->tanggal_deadline->format('d M Y, H:i') }}</div>
        </div>
    ...
    <!-- Deskripsi Tugas -->
    <div style="background: #f0f9ff; padding: 20px; border-radius: 8px; border-left: 4px solid #165fac; margin-bottom: 25px;">
    ...
    <div style="background: white; padding: 25px; border: 2px solid #e5e7eb; border-radius: 12px;">
        ...
        <form action="{{ route('siswa.lms.mapel.tugas.submit', [$mataPelajaran->id, $tugas->id]) }}" 
              method="POST" 
            ...
            <div class="form-group mb-3">
                ...
                <textarea name="jawaban_text" 
                          rows="8" 
                          class="form-control @error('jawaban_text') is-invalid @enderror"
                          placeholder="Tulis jawaban Anda di sini..."
                          {{ $isExpired ? 'disabled' : '' }}>{{ old('jawaban_text', $existingSubmission->jawaban_text ?? '') }}</textarea>
                @error('jawaban_text')
            ...
            <div class="form-group mb-3">
                ...
                <input type="file" 
                       name="file_jawaban" 
                       class="form-control @error('file_jawaban') is-invalid @enderror"
                ...
                <div class="mt-2">
                    ...
                    <a href="{{ asset('storage/' . $existingSubmission->file_jawaban) }}" 
                       target="_blank" 
        ...
        <div class="alert alert-info mt-3" role="alert">
            <small>
                ...
                Terakhir disubmit: {{ $existingSubmission->tanggal_submit->format('d F Y, H:i') }} WIB
