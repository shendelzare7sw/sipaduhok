...
<style>
    ...
    .badge-field_trip { background: #dbeafe; color: #1e40af; }
    .badge-outing { background: #fef3c7; color: #92400e; }
    .badge-live_in { background: #fce7f3; color: #9f1239; }
    .badge-hokfest { background: #e0e7ff; color: #3730a3; }
    .badge-pts { background: #fed7aa; color: #9a3412; }
    .badge-pas { background: #fecaca; color: #991b1b; }
    .badge-libur { background: #d1fae5; color: #065f46; }
    .badge-ujian { background: #fecaca; color: #991b1b; }
    .badge-acara_sekolah { background: #e0e7ff; color: #3730a3; }
...
<!-- Back Button -->
<div class="mb-3">
    <a href="{{ route('siswa.lms.kalender.index') }}" class="btn btn-secondary">
        <i class="fas fa-arrow-left"></i> Kembali ke Kalender
    </a>
</div>

<!-- Date Header -->
<div class="content-card" style="background: linear-gradient(135deg, #165fac, #0d3f7a); color: white; margin-bottom: 20px;">
...
<div class="event-card" style="border-color: {{ $event->jenis_kegiatan === 'pts' || $event->jenis_kegiatan === 'pas' ? '#ef4444' : '#165fac' }};">
...
<div class="d-flex justify-content-between mt-4">
    @if($prevDate)
    <a href="{{ route('siswa.lms.kalender.detail', ['date' => $prevDate->format('Y-m-d')]) }}" 
    ...
    @if($nextDate)
    <a href="{{ route('siswa.lms.kalender.detail', ['date' => $nextDate->format('Y-m-d')]) }}" 
       class="btn btn-secondary">
