...
@media (max-width: 767.98px) {
    ...
    .nav-tabs .nav-link { font-size: 11px; padding: 6px 8px; }
}
...
<div class="container-fluid px-0">
    ...
    <div class="alert alert-light border border-primary border-opacity-25 shadow-sm mb-4">
        <div class="d-flex align-items-center gap-2 flex-wrap">
            ...
            <span class="badge bg-primary"><i class="fas fa-money-bill me-1"></i>2. Bendahara Validasi</span>
            <i class="fas fa-arrow-right text-muted small d-none d-sm-inline"></i>
    ...
    <div class="card shadow mb-3" style="position: relative; z-index: 99;">
        <div class="card-body py-2">
            <form action="{{ route('bendahara.validasi-akses.index') }}" method="GET" class="d-flex gap-2 flex-wrap filter-row">
                <div class="dropdown">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($siswa as $index => $s)
                                <tr>
                                    <td class="text-center align-middle"><input type="checkbox" name="siswa_ids[]" value="{{ $s->id }}" class="siswa-checkbox form-check-input" data-ketua-approved="{{ $s->validasi_rapor_ketua ? '1' : '0' }}"></td>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 bg-white d-flex justify-content-between align-items-center">
            <h6 class="m-0 fw-bold text-primary"><i class="fas fa-cog me-2"></i>Pengaturan Batas Pembayaran Per Periode</h6>
            <span class="badge bg-info">Tahun Ajaran: {{ $tahunAjaran->nama ?? '-' }}</span>
        </div>
        <div class="card-body">
            <p class="text-muted small mb-3">Tentukan jenis tagihan yang harus lunas agar siswa otomatis mendapat akses ujian/rapor. Jika tidak ada yang dipilih, semua siswa dianggap lunas untuk periode tersebut.</p>
            ...
            <div class="tab-content border border-top-0 rounded-bottom p-3" id="periodeTabContent">
                @foreach($periodeList as $key => $label)
                        ...
                        <form action="{{ route('bendahara.validasi-akses.batas-pembayaran') }}" method="POST">
                            ...
                            <div class="row">
                                ...
                                @foreach($grouped as $groupLabel => $items)
                                    <div class="col-md-6">
                                        <h6 class="fw-bold text-secondary mb-2">{{ $groupLabel }}</h6>
                            ...
                            <div class="mt-3">
                                <button type="submit" class="btn btn-primary btn-sm fw-bold"><i class="fas fa-save me-1"></i> Simpan {{ $label }}</button>
                            </div>
                        </form>
                    </div>
                @endforeach
...
function bulkValidasiRapor() {
    ...
    // Check how many have ketua approval
    ...
    if (eligible.length === 0) {
        showAlertModal('Tidak Bisa Validasi Rapor', 'Semua siswa terpilih belum di-approve Ketua PKBM. Validasi rapor hanya bisa dilakukan setelah Ketua menyetujui. Silakan pilih siswa yang sudah berstatus "BELUM" (bukan "TUNGGU KETUA").');
        return;
    }
    ...
    showConfirmModal('Validasi Akses Rapor', msg, function() {
