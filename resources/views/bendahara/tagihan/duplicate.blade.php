            ...
            <div class="alert alert-info border-start border-info border-4 shadow-sm mb-4">
                <div class="d-flex">
                    ...
                    <div>
                        ...
                        <p class="mb-0 mt-1">Fitur ini memungkinkan Anda menyalin semua tagihan dari satu siswa ke siswa
                            lain secara massal. Sangat berguna saat membuat tagihan untuk siswa baru atau satu kelas.</p>
                    </div>
                </div>
            </div>
                    ...
                    <form action="{{ route('bendahara.tagihan.duplicate.store') }}" method="POST" id="duplicateForm">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="source_siswa_id" id="source_siswa_id"
                                    class="form-select @error('source_siswa_id') is-invalid @enderror"
                                    data-preview-url="{{ route('bendahara.tagihan.api.tagihan-preview', ':siswa') }}" required>
                                ...
                                <div class="card bg-light">
                                    <div class="card-body">
                                        ...
                                        <div id="preview_tagihan">
                                            <p class="text-muted mb-0"><em>Pilih siswa sumber untuk melihat preview
                                                    tagihan</em></p>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        @foreach($siswaList as $siswa)
                                            <div class="col-md-6 target-siswa-item" data-kelas-id="{{ $siswa->kelas_id }}">
                                                <div class="form-check">
                                                    <input class="form-check-input target-siswa-checkbox" type="checkbox"
                                                        name="target_siswa_ids[]" value="{{ $siswa->id }}"
                                                        id="siswa_{{ $siswa->id }}">
                                                    <label class="form-check-label" for="siswa_{{ $siswa->id }}">
                                                        <strong>{{ $siswa->nama_lengkap }}</strong><br>
                                                        <small class="text-muted">{{ $siswa->nisn }} -
                                                            {{ $siswa->kelas->nama_kelas ?? '-' }}</small>
                                                    </label>
                                                </div>
                                            </div>
                                        @endforeach
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="replace_existing"
                                            id="replace_yes" value="1" {{ old('replace_existing', '1') == '1' ? 'checked' : '' }}>
                                        <label class="form-check-label" for="replace_yes">
                                            <strong>Replace</strong> - Perbarui dengan nominal baru
                                        </label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="replace_existing" id="replace_no"
                                            value="0" {{ old('replace_existing') == '0' ? 'checked' : '' }}>
                                        <label class="form-check-label" for="replace_no">
                                            <strong>Skip</strong> - Lewati jika sudah ada
                                        </label>
                                    </div>
                                </div>
                                <small class="text-muted">Tentukan aksi jika siswa target sudah memiliki tagihan dengan
                                    jenis yang sama</small>
                            </div>
                        </div>
