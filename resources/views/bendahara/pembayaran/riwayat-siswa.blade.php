...
@section('page-subtitle', 'Riwayat pembayaran {{ $siswa->nama_lengkap }}')
...
<style>
    ...
    .stat-card-mini h6 { font-size: 12px; opacity: 0.9; margin-bottom: 8px; }
    .stat-card-mini .stat-value { font-size: 24px; font-weight: 700; margin-bottom: 4px; }
    .stat-card-mini .stat-label { font-size: 11px; opacity: 0.8; }
    .stat-card-mini .stat-icon { position: absolute; right: 15px; top: 50%; transform: translateY(-50%); font-size: 50px; opacity: 0.2; }
    .timeline-dot { width: 12px; height: 12px; border-radius: 50%; position: absolute; left: -30px; top: 4px; }
    .timeline-line { position: absolute; left: -25px; top: 0; bottom: 0; width: 2px; background: #e5e7eb; }
</style>
@endsection
                        ...
                        <tbody>
                            @foreach($pembayaran as $bayar)
                                <tr>
                                    ...
                                    <td class="align-middle text-center">
                                        <a href="{{ route('bendahara.pembayaran.show', $bayar->id) }}"
            ...
            <div class="position-relative" style="padding-left: 30px;">
                @foreach($pembayaran->take(10) as $bayar)
                    <div class="position-relative pb-4">
                        ...
                        {{-- Dot --}}
                        <div class="timeline-dot {{ $bayar->status_validasi === 'disetujui' ? 'bg-success' : ($bayar->status_validasi === 'ditolak' ? 'bg-danger' : 'bg-warning') }}">
                        </div>
                        ...
                        <div class="bg-light p-3 rounded shadow-sm">
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    <strong class="text-dark">Rp {{ number_format($bayar->jumlah_bayar, 0, ',', '.') }}</strong>
