/* Page asset: wali-kelas/presensi/rekap-harian. */
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('[data-progress-width]').forEach(function (bar) {
        const width = parseFloat(bar.dataset.progressWidth);
        bar.style.width = (Number.isFinite(width) ? Math.max(0, Math.min(width, 100)) : 0) + '%';
    });
});
